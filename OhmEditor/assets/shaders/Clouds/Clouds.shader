...
void main()
{
    // Put the coordinate into view space.  Keep w as 1.0 to maintain z information.
	vec3 viewVector = (inverse(ProjectionMatrix) * vec4(a_TexCoord * 2.0 - 1.0, 0.0, 1.0)).xyz;
    // Put the coordinate into world space, but convert it to a direction.
	v_WorldSpaceViewDirection = (inverse(ViewMatrix) * vec4(viewVector, 0.0)).xyz;
	v_UV = a_TexCoord;
...
float SampleDensity(vec3 rayPosition)
{
    ...
    float time = u_TimeScale * ElapsedTime;
...
float LightMarch(vec3 hitPoint)
{
    ...
    vec3 inViewVector = normalize(hitPoint - CameraPosition.xyz);
...
void main()
{
    ...
    {
        vec3 rayPosition = CameraPosition.xyz;
        ...
        if (u_DrawClouds)
        {
            ...
            float randomRayOffset = texture(u_BlueNoiseTexture, v_UV).r;
            ...
            cloudColor = lightEnergy * LightRadiance;
        }
        ...
        color = vec4(sceneColor * transmittance + cloudColor + LightRadiance * sunValue, 1.0);
    }
